...
public class MinesweeperGame {
    ...
    private static void actOnCell(String cellInput, String userActionInput) {
        ...
        if (doesUserChooseToPlantFlag(userActionInput)) {
            ...
            checkIfGameIsOver();    // 게임이 끝난는지 확인하는 과정
            return;
        }
        ...
        throw new AppException("잘못된 번호 선택하셨습니다");
    }
    ...
    private static void checkIfGameIsOver() {
        boolean isAllOpened = isAllCellOpened();    // 모든 셀이 열렸는지?
        if (isAllOpened) {
...
//    }
    ...
    private static boolean isAllCellOpened() {
        return Arrays.stream(BOARD) //Stream<String[]>
                ...
                .noneMatch(cell -> cell.equalsSign(CLOSED_CELL_SIGN)); // cell.equals(CLOSED_CELL_SIGN) : NPE 가능성 있음, CLOSED_CELL_SIGN.equals(cell) : 가능성 X 상수니깐.
    }
    ...
    private static void initializeGame() {
        ...
        for (int i = 0; i < LAND_MINE_COUNT; i++) {
            ...
            LAND_MINES[row][col] = true; // 지뢰 초기화, isLandMineCell은 지뢰 밟았냐? 확인하는 메서드 두개 의미가 다름
        }
